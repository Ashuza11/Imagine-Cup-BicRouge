...
import { ClipboardPenLine, GripHorizontal, X } from "lucide-react";
import {
    forwardRef,
    useCallback,
    useContext,
...
import {
    FetchAllQuestions,
    SubmitAssignmentResponsesQuery,
} from "../query/AssignmentQuery";
...
const ResponseForm = () => {
    ...
    const [, setCardsState] = useState(cards);
    const { assignmentsStudent } = useAssignmentsStudent(courseId);
    const currentAssignement = assignmentsStudent?.find(
    ...
    const handleSubmit = (e) => {
        ...
        const allFieldsFilled = allRef.every(
            (card) => card.getValue() && card.getValue().trim() !== ""
        );
    ...
    const forcedSubmit = () => {
        const questions = allRef.map((ref, index) => ({
            id: index + 1,
            responseInput: ref.getValue() || "Je ne sais pas",
        }));
...
const QuestionCard = forwardRef(function QuestionCard(
    { question, t, questionId, isFirstCardClicked },
    ref
) {
    ...
    return (
        <div
            ...
            className={`bg-white shadow-lg p-4 rounded-lg w-full mb-5 transform transition-all duration-300 ease-in-out ${
                isClicked
                    ? "border-l-8 border-red-600 scale-y-105"
                    : "scale-100"
            }`}
        ...
        >
            <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center space-x-2">
                    <span className="text-xl">/{maxPoints}</span>
                </div>
            </div>
            <div className="flex flex-col space-y-2 gap-2">
                ...
                <div>
                    ...
                    <br />
                    <span className="font-italic text-secondary w-full inline-block pb-1 border-b-2 border-dashed">
                        {response}
                    </span>
                </div>
            </div>
