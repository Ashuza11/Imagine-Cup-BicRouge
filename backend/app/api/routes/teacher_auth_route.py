...
from models.user_models import Teacher
...
@router.post("/token", response_model=Token, operation_id="login_teacher_token")
async def login(
    form_data: OAuth2PasswordRequestForm = Depends(),
    db: Session = Depends(get_db),
):
    user = await authenticate_user(form_data.username, form_data.password, db, Teacher)
    if not user:
...
async def read_users_me(current_user: Teacher = Depends(check_user_active)):
    return current_user
